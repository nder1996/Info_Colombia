...
export default function GestionTareas() {
  ...
  const agregarTarea = async (data?: dataTarea) => {
    try {
      ...
      console.log("tareaRequest: ", tareaRequest)
      const response = await GestionTareasService.createTarea(tareaRequest);
      if (response && response.data && response.meta?.statusCode == 200) {
        mostrarExito("Tarea Agregada Correctamente", "La tarea se agregó correctamente");
        reset();
        setVisible(false);
      } else {
        mostrarAdvertencia("Error al Agregar la Tarea", "Hubo un error al agregar la tarea");
      }*/
    } catch (error) {
      console.error('Error:', error);
