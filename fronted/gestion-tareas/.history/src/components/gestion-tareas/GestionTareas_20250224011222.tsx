...
export default function GestionTareas() {
  ...
  const actualizarTarea = async (data: dataTarea) => {
    try {
      ...
      const tareaRequest: Tarea = {
        ...
        titulo: data.titulo,
        descripcion: data.descripcion,
        username: selectedUser ?? sessionInfo?.username ?? undefined,
        idEstadoTarea: data.estado,
        vencimientoTarea: data.fechaVencimiento
      };
  ...
  const fechaUnDiaMax = (): Date => {
    const tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
    return tomorrow;
  };

  const [selectedDate, setSelectedDate] = useState<Date>(fechaUnDiaMax());
        ...
        <form onSubmit={handleSubmit(eventoSelectAccion)} className="form-container">
          ...
          <div className="form-group">
            ...
            <Calendar
              ...
              className="form-date"
              value={selectedDate}
              minDate={minSelectableDate} // Restringe a mañana en adelante
              onChange={(e) => {
