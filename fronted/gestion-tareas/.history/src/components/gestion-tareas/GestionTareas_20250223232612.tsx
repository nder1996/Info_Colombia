...
export default function GestionTareas() {
  ...
  const buttonAgregar = async (e: React.FormEvent) => {
    try {
      ...
      const idEstado = summaryData[0]?.id ?? 1;
      console.log("estado precargado: ", idEstado)
  ...
  const agregarTarea = async (data: dataTarea) => { // Elimina el opcional ?
    try {
      if (!data) {
        mostrarError("Error", "No hay datos para crear la tarea");
        return;
      }
      ...
      const tareaRequest: Tarea = {
        ...
        idUsuario: selectedUser === null ? sessionInfo?.username : selectedUser,
      ...
      if (response?.data && response.meta?.statusCode === 200) {
        mostrarExito("Tarea Agregada Correctamente", "La tarea se agregó correctamente");
        await fetchTareas();
        reset();
        setVisible(false);
      } else {
        mostrarAdvertencia("Error al Agregar la Tarea", response.meta?.message || "Error al agregar la tarea");
      }
    } catch (error) {
      console.error('Error:', error);
      mostrarError("Error en el Sistema", "Ocurrió un error al procesar la tarea");
    }
  };
