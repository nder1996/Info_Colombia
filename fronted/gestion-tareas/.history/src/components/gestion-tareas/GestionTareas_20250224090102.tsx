...
export default function GestionTareas() {
  ...
  const schema = yup.object({
    id: yup.number(),
    titulo: yup.string().required(t('validaciones.tituloRequerido')),
    descripcion: yup.string().required(t('validaciones.descripcionRequerida')),
    estado: yup.number().required(t('validaciones.estadoRequerido')),
  ...
  const loadSummaryData = async () => {
    try {
      ...
      if (data && data.data && data.meta?.statusCode == 200) {
        ...
        mostrarAdvertencia(t('mensajes.advertencia.errorCargarTareas'), tadatarea.meta?.message);
      }
    } catch (error) {
      console.error('Error loading summary:', error);
    }
  ...
  const confirmarEliminarTarea = (tarea: TareasResponse) => {
    confirmDialog({
      message: `¿Está seguro que desea eliminar la tarea "${tarea.titulo}"?`,
      ...
      reject: () => { } // No hace nada si el usuario cancela
    });
  };
  ...
  const bottonEliminar = async (tarea: TareasResponse) => {
    ...
    try {
      ...
      if (response && response?.meta?.statusCode === 200) {
        mostrarExito("Tarea Eliminada", `Tarea ${tarea.titulo} eliminada`);
      } else {
        mostrarError("Error", response?.meta?.message || "Error al inactivar tarea");
      }
    } catch (error) {
      console.error('Error al editar el registro:', error);
    }
...
function useTranslations(): { t: any; currentLanguage: any; } {
