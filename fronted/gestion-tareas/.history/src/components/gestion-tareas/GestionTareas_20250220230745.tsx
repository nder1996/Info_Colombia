...
export default function GestionTareas() {
  ...
  const editarRegistro = async (tarea: TareasResponse) => {
    try {
      // Espera a que loadSummaryData termine
      await loadSummaryData();
  
      // Actualiza el estado formData con los datos de la tarea
      setFormData({
        id: tarea.id,
        titulo: tarea.titulo,
        descripcion: tarea.descripcion,
        fechaVencimiento: new Date(), // ¿Debe ser tarea.fechaVencimiento en lugar de new Date()?
        estado: tarea.estadoTarea.id,
        usuarioAsignado: '', // ¿Debe ser tarea.usuarioAsignado en lugar de ''?
      });
  
      // Muestra el formulario o modal
      setVisible(true);
  
      // Cambia el tipo de evento a "editar"
