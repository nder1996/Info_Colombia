...
export default function GestionTareas() {
  ...
  const [rolAdmin, setRolAdmin] = useState<RolAdminState | null>(null);
    ...
    const fetchRoles = async () => {
      ...
      if (sessionInfo?.username) {
        ...
        if(rol && rol.meta?.statusCode==200) {
          const rolAdmin = Array.isArray(rol.data)
           ? rol.data.find(rol => rol.id == 1)
           : "No hay roles asociados al usuario";
           setRolAdmin({
            rolAdmin
           })
        }
