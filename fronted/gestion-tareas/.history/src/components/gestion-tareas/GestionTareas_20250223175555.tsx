  ...
  */
  ...
  useEffect(() => {
    if (notifications.length > 0 && toastRef.current) {
      const notification = notifications[0] ?? []; // or however you're selecting the specific notification
      toastRef?.current.show({
        severity: 'info',
  ...
  return (
    <div className="card" style={{
      ...
      <DataTable
        ...
        filterDisplay="menu"
      >
        <Column showFilterMenu={false} field="id" header="ID" filter filterPlaceholder="Buscar por ID" style={{ width: '5%' }}></Column>
        <Column showFilterMenu={false} field="titulo" header="Título" filter filterPlaceholder="Buscar por título" style={{ width: '20%' }}></Column>
        <Column showFilterMenu={false} field="descripcion" header="Descripción" filter filterPlaceholder="Buscar por descripción" style={{ width: '25%' }}></Column>
        <Column
