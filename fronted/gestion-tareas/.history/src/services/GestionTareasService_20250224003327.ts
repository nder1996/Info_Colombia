...
export class GestionTareasService {
    ...
    static async createTarea(tarea: Tarea): Promise<ApiResponse<string>> {
        alert("baseURL TAREA : "+this.api)
        try {
            alert("Tarea creada: " + JSON.stringify(InterceptorAuth.tarea) + "  /  " + "createTarea");
            const response = await InterceptorAuth.tarea.post('createTarea', tarea);
            console.log('Tarea creada createTarea :', response.data);
            return response.data;
        } catch (error) {
    ...
    static async updateTask(task: Tarea): Promise<string | null> {
        try {
            const response = await InterceptorAuth.tarea.put('updateTarea', task);
            return response.data;
        } catch (error) {
