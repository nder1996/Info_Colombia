...
export class SummaryService {
    ...
    static async obtenerSummary(): Promise<ApiResponse<EstadoTarea[]>> {
        try {
            const response = await InterceptorAuth.auth.get<ApiResponse<EstadoTarea[]>>('getAllData');
    ...
    static async obtenerSummaryPorId(id: number): Promise<ApiResponse<EstadoTarea>> {
        try {
            const response = await InterceptorAuth.auth.get<ApiResponse<EstadoTarea>>(`getByData/${id}`);
            return response.data;
        } catch (error) {
            console.error('Error:', error);
            return {
