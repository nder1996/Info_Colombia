...
export class AuthService {
    ...
    public static getToken(): string {
     ...
     public static getInfoSession(): AuthData {
        const authData = LocalStorageService.get('authToken');
        if (!authData) {
            return new AuthData(); // Return a default instance if no data is found
        }
        // If authData is a JSON string, parse it
        if (typeof authData === 'string') {
            return JSON.parse(authData) as AuthData;
        }
        // If authData is already an object, return it
